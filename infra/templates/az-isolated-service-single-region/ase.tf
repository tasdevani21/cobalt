...
module "app_service" {
  ...
  docker_registry_server_password  = format("@Microsoft.KeyVault(SecretUri=%s)", "${module.keyvault.keyvault_uri}secrets/${module.acr_service_principal_password.keyvault_secret_names[0]}/${module.acr_service_principal_password.keyvault_secret_versions[0]}") #data.azurerm_key_vault_secret.acr_password.id)
  app_service_config = {
